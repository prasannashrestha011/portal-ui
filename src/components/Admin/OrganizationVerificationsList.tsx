...
> = {
  ...
  approved: { badge: "bg-green-50 text-green-700", label: "Verified" },
...
export function OrganizationVerificationsListPage({ status }: { status?: string }) {
  console.log("OrganizationVerificationsListPage status:", status);
  ...
  const { data: verifications, isLoading, error } = useQuery({
    queryKey: ["org-verifications", currentPage, pageSize, status],
    queryFn: () =>
  ...
  const counts = useMemo(() => {
    ...
    return verifications.data.reduce(
      ...
      { pending: 0, approved: 0, rejected: 0 } as Record<OrganizationVerificationStatus, number>
    );
  }, [verifications]);
  ...
  const openReview = (verification: OrganizationVerification, action: "approved" | "rejected") => {
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white px-4 py-3 rounded-lg border border-slate-200 shadow-sm">
              <div className="flex items-center gap-4 text-sm">
                ...
                <span className="font-medium text-green-700">{counts.approved} verified</span>
                ...
                return (
                  ...
                  >
                    <div className="p-5 sm:p-6 sm:flex sm:items-center sm:justify-between gap-6">
                      ...
                      <Link href={`organization-verifications/${verification.id}`} className="text-indigo-600 hover:text-indigo-800">
                        View Details
                      </Link>
                      ...
                      {verification.status === "pending" && (
                        <div className="mt-4 sm:mt-0 flex gap-2 shrink-0">
                          ...
                          <Button
                            ...
                            onClick={() => openReview(verification, "approved")}
                          >
